			...
			if (button && button.element) {
				...
				if (btn.hover) {
					for (const propName in btn.hover) {
						const hover = btn.hover[propName];
						const normal = button.element.style[changeCase.camelCase(propName)];

						// Add listeners so hover behavior is simulated
						button.element.onmouseenter = function () {
							button.element.style[changeCase.camelCase(propName)] = hover;
						};

						button.element.onmouseleave = function () {
							button.element.style[changeCase.camelCase(propName)] = normal;
						};
					}
				}
