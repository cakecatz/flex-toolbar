...
export default {
	...
	resolveConfigPath(configFilePath, createIfNotFound) {
		...
		if (configPath) {
			...
			if ((exists && fs.isDirectorySync(configPath)) || (!exists && (needle = path.extname(configPath), !VALID_EXTENSIONS.includes(needle)))) {
	...
	createConfig(configPath) {
		try {
			...
			const content = fs.readFileSync(path.resolve(__dirname, `./default/toolbar${ext}`));
	...
	registerEvents() {
		...
		return this.subscriptions.add(atom.workspace.onDidChangeActivePaneItem(() => {
	...
	registerTypes() {
		...
		return typeFiles.forEach(typeFile => {
			const typeName = path.basename(typeFile, '.coffee');
