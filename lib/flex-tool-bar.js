...
export default {
	...
	resolveConfigPath(configFilePath, createIfNotFound) {
		...
		if (configPath) {
			...
			if ((exists && fs.isDirectorySync(configPath)) || (!exists && !VALID_EXTENSIONS.includes(path.extname(configPath)))) {
				configPath = path.resolve(configPath, 'toolbar.js');
			}
	...
	onDidChangeItem() {
		...
		if (this.activeItem.item !== active.item) {
	...
	registerWatch(shouldWatch) {
		...
		this.configWatcher = chokidar.watch(this.configFilePath).on('change', () => {
	...
	registerProjectWatch(shouldWatch) {
		...
		this.projectConfigWatcher = chokidar.watch(this.projectConfigFilePath).on('change', () => {
			this.reloadToolbar(atom.config.get('flex-tool-bar.reloadToolBarNotification'));
		});
	},
	...
	functionCondition(condition) {
		...
		const value = !!condition(this.activeItem.item);
	...
	getActiveItem() {
		...
		if (active.file) {
			[active.project] = atom.project.relativizePath(active.file);
		}
	...
	grammarCondition(condition) {
		...
		return this.reversableStringCondition(condition, (c) => {
			return this.activeItem.grammar && this.activeItem.grammar.includes(c.toLowerCase());
		});
	},
	...
	patternCondition(condition) {
		...
		return this.reversableStringCondition(condition, (c) => {
			return this.activeItem.file && globToRegexp(c, {extended: true}).test(this.activeItem.file);
		});
	},
